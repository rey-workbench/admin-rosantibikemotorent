        ...
        {#if currentView === "chat-detail"}
          ...
          >
            ...
            <ChatInput
              ...
              onTyping={(t) =>
                t
                  ? whatsappApi.startTyping(selectedContact.phone)
                  : whatsappApi.stopTyping(selectedContact.phone)}
            />
          </div>
        {:else}
          <div class="flex-1 flex flex-col h-full bg-bg-secondary" in:fade>
            <ChatHeader
            ...
            {#if sessionStatus === "qr_timeout"}
              ...
              >
                ...
                <p class="text-sm text-text-muted max-w-xs mb-6">
                  QR Code tidak di-scan dalam 3 percobaan. Klik tombol di bawah
                  untuk meminta QR baru.
                </p>
                <button
            ...
            {:else if sessionStatus !== "connected" && qrCode}
              ...
              >
                ...
                <p class="text-sm text-text-muted max-w-xs">
                  Buka WhatsApp di ponsel Anda, buka Pengaturan &gt; Perangkat
                  Tertaut, dan scan kode ini.
                </p>
              </div>
            {/if}
            <ContactList
