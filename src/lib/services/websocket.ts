...
type WhatsAppQrCodeHandler = (data: {
  qrCode: string | null;
  timestamp: Date;
}) => void;
type QueueUpdateHandler = (data: QueueUpdate) => void;
type HttpCompleteHandler = (data: {
  requestId: string;
  timestamp: Date;
  result: unknown;
}) => void;
...
class WebSocketService {
  ...
  connect(): void {
    ...
    if (!apiUrl) {
      if (
        browser &&
        window.location.hostname.includes("rosantibikemotorent.com")
      ) {
  ...
  private setupEventListeners(): void {
    ...
    this.socket.on("whatsapp:qrcode", (data: unknown) => {
      ...
      this.whatsappQrCodeHandlers.forEach((handler) =>
        handler({ qrCode, timestamp: new Date() }),
      );
    });
