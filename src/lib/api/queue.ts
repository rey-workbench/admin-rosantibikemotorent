...
export const queueApi = {
  ...
  getJobs: async (
    ...
    const { data } = await api.get(`/debug/queue/${queueName}/jobs`, {
      params: { status, start, size },
    });
    return data.data;
  },
  ...
  cancelJob: async (
    ...
    jobId: string,
    reason?: string,
  ): Promise<{ ok: boolean; mode: string; state: string; reason: string }> => {
    const { data } = await api.post(
      `/debug/queue/${queueName}/jobs/${jobId}/cancel`,
      {
        reason,
      },
    );
