<script lang="ts">
import { AlertCircle, ChevronLeft, ChevronRight, Filter } from '@lucide/svelte';
...
async function loadAvailability() {
	...
	try {
		...
		const endDate = new Date(currentYear, currentMonth, daysInMonth).toISOString().split('T')[0];

		let params: any = { startDate, endDate };
		if (unitTypeFilter) params.jenisId = unitTypeFilter;
		availabilityData = await unitMotorApi.checkAvailability(params as any);
	} catch (e: any) {
		toast.error(e);
